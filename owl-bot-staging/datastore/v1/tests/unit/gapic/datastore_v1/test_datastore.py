...
def test__get_default_mtls_endpoint():
    ...
    assert DatastoreClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert DatastoreClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert DatastoreClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test__read_environment_variables():
    ...
    with mock.patch.dict(os.environ, {"GOOGLE_CLOUD_UNIVERSE_DOMAIN": "foo.com"}):
        assert DatastoreClient._read_environment_variables() == (False, "auto", "foo.com")
...
def test__get_client_cert_source():
    ...
    assert DatastoreClient._get_client_cert_source(mock_provided_cert_source, False) is None
    assert DatastoreClient._get_client_cert_source(mock_provided_cert_source, True) == mock_provided_cert_source

    with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
        with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_default_cert_source):
            assert DatastoreClient._get_client_cert_source(None, True) is mock_default_cert_source
            assert DatastoreClient._get_client_cert_source(mock_provided_cert_source, "true") is mock_provided_cert_source

@mock.patch.object(DatastoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatastoreClient))
@mock.patch.object(DatastoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatastoreAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    mock_client_cert_source = mock.Mock()
    default_universe = DatastoreClient._DEFAULT_UNIVERSE
    default_endpoint = DatastoreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=default_universe)
    mock_universe = "bar.com"
    mock_endpoint = DatastoreClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=mock_universe)

    assert DatastoreClient._get_api_endpoint(api_override, mock_client_cert_source, default_universe, "always") == api_override
    assert DatastoreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "auto") == DatastoreClient.DEFAULT_MTLS_ENDPOINT
    assert DatastoreClient._get_api_endpoint(None, None, default_universe, "auto") == default_endpoint
    assert DatastoreClient._get_api_endpoint(None, None, default_universe, "always") == DatastoreClient.DEFAULT_MTLS_ENDPOINT
    assert DatastoreClient._get_api_endpoint(None, mock_client_cert_source, default_universe, "always") == DatastoreClient.DEFAULT_MTLS_ENDPOINT
    assert DatastoreClient._get_api_endpoint(None, None, mock_universe, "never") == mock_endpoint
    assert DatastoreClient._get_api_endpoint(None, None, default_universe, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        DatastoreClient._get_api_endpoint(None, mock_client_cert_source, mock_universe, "auto")
...
def test__get_universe_domain():
    ...
    assert DatastoreClient._get_universe_domain(client_universe_domain, universe_domain_env) == client_universe_domain
    assert DatastoreClient._get_universe_domain(None, universe_domain_env) == universe_domain_env
    assert DatastoreClient._get_universe_domain(None, None) == DatastoreClient._DEFAULT_UNIVERSE
...
def test_datastore_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (DatastoreClient, transports.DatastoreGrpcTransport, "grpc"),
    (DatastoreAsyncClient, transports.DatastoreGrpcAsyncIOTransport, "grpc_asyncio"),
    (DatastoreClient, transports.DatastoreRestTransport, "rest"),
])
@mock.patch.object(DatastoreClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatastoreClient))
@mock.patch.object(DatastoreAsyncClient, "_DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(DatastoreAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    ...
    (DatastoreAsyncClient, transports.DatastoreGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (DatastoreClient, transports.DatastoreGrpcTransport, "grpc", "false"),
    (DatastoreAsyncClient, transports.DatastoreGrpcAsyncIOTransport, "grpc_asyncio", "false"),
...
def test_datastore_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
@pytest.mark.parametrize("client_class", [
    DatastoreClient, DatastoreAsyncClient
])
@mock.patch.object(DatastoreClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DatastoreClient))
@mock.patch.object(DatastoreAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(DatastoreAsyncClient))
...
def test_datastore_client_client_options_scopes(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    ...
    (DatastoreAsyncClient, transports.DatastoreGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
    (DatastoreClient, transports.DatastoreRestTransport, "rest", None),
])
def test_datastore_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_datastore_client_client_options_from_dict():
    with mock.patch('google.cloud.datastore_v1.services.datastore.transports.DatastoreGrpcTransport.__init__') as grpc_transport:
...
def test_datastore_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
async def test_lookup_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.LookupResponse(
            transaction=b'transaction_blob',
        ))
...
def test_lookup_flattened():
    ...
    with mock.patch.object(
        ...
        client.lookup(
            project_id='project_id_value',
            read_options=datastore.ReadOptions(read_consistency=datastore.ReadOptions.ReadConsistency.STRONG),
            keys=[entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))],
        )
        ...
        mock_val = datastore.ReadOptions(read_consistency=datastore.ReadOptions.ReadConsistency.STRONG)
        assert arg == mock_val
        arg = args[0].keys
        mock_val = [entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))]
        assert arg == mock_val
...
async def test_run_query_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.RunQueryResponse(
            transaction=b'transaction_blob',
        ))
...
async def test_run_query_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._client._transport.run_query in client._client._transport._wrapped_methods
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.run_query] = mock_object
...
def test_run_aggregation_query(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.run_aggregation_query),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_run_aggregation_query_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert client._transport.run_aggregation_query in client._transport._wrapped_methods

        # Replace cached wrapped function with mock
        mock_rpc = mock.Mock()
        mock_rpc.return_value.name = "foo" # operation_request.operation in compute client(s) expect a string.
        client._transport._wrapped_methods[client._transport.run_aggregation_query] = mock_rpc
        request = {}
...
async def test_run_aggregation_query_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.RunAggregationQueryResponse(
            transaction=b'transaction_blob',
        ))
...
@pytest.mark.asyncio
async def test_run_aggregation_query_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DatastoreAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.run_aggregation_query in client._client._transport._wrapped_methods
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.run_aggregation_query] = mock_object
...
@pytest.mark.asyncio
async def test_run_aggregation_query_async(transport: str = 'grpc_asyncio', request_type=datastore.RunAggregationQueryRequest):
    client = DatastoreAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
async def test_commit_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.CommitResponse(
            index_updates=1389,
        ))
...
def test_commit_flattened():
    ...
    with mock.patch.object(
        ...
        client.commit(
            ...
            transaction=b'transaction_blob',
            mutations=[datastore.Mutation(insert=entity.Entity(key=entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))))],
        )
        ...
        mock_val = [datastore.Mutation(insert=entity.Entity(key=entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))))]
        assert arg == mock_val
...
async def test_rollback_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.RollbackResponse(
        ))
...
@pytest.mark.asyncio
async def test_allocate_ids_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DatastoreAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.allocate_ids in client._client._transport._wrapped_methods
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.allocate_ids] = mock_object
...
async def test_reserve_ids_empty_call_async():
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(datastore.ReserveIdsResponse(
        ))
...
@pytest.mark.asyncio
async def test_reserve_ids_async_use_cached_wrapped_rpc(transport: str = "grpc_asyncio"):
    # Clients should use _prep_wrapped_messages to create cached wrapped rpcs,
    # instead of constructing them on each call
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        client = DatastoreAsyncClient(
        ...
        # Ensure method has been cached
        assert client._client._transport.reserve_ids in client._client._transport._wrapped_methods
        ...
        class AwaitableMock(mock.AsyncMock):
            def __await__(self):
                ...
                return iter([])
        mock_object = AwaitableMock()
        client._client._transport._wrapped_methods[client._client._transport.reserve_ids] = mock_object
...
def test_lookup_rest_unset_required_fields():
    transport = transports.DatastoreRestTransport(credentials=ga_credentials.AnonymousCredentials)

    unset_fields = transport.lookup._get_unset_required_fields({})
    assert set(unset_fields) == (set(()) & set(("projectId", "keys", )))
...
def test_lookup_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = datastore.LookupResponse.to_json(datastore.LookupResponse())
...
def test_lookup_rest_bad_request(transport: str = 'rest', request_type=datastore.LookupRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_lookup_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1/projects/{project_id}:lookup" % client.transport._host, args[1])
...
def test_run_query_rest_required_fields(request_type=datastore.RunQueryRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).run_query._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_run_query_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DatastoreRestInterceptor, "post_run_query") as post, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.RunQueryResponse.to_json(datastore.RunQueryResponse())
        ...
        client.run_query(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_run_query_rest_bad_request(transport: str = 'rest', request_type=datastore.RunQueryRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_run_aggregation_query_rest_required_fields(request_type=datastore.RunAggregationQueryRequest):
    transport_class = transports.DatastoreRestTransport
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).run_aggregation_query._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_run_aggregation_query_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DatastoreRestInterceptor, "post_run_aggregation_query") as post, \
         mock.patch.object(transports.DatastoreRestInterceptor, "pre_run_aggregation_query") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = datastore.RunAggregationQueryRequest.pb(datastore.RunAggregationQueryRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.RunAggregationQueryResponse.to_json(datastore.RunAggregationQueryResponse())
        ...
        client.run_aggregation_query(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_run_aggregation_query_rest_bad_request(transport: str = 'rest', request_type=datastore.RunAggregationQueryRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_begin_transaction_rest_required_fields(request_type=datastore.BeginTransactionRequest):
    transport_class = transports.DatastoreRestTransport
...
def test_begin_transaction_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DatastoreRestInterceptor, "post_begin_transaction") as post, \
         mock.patch.object(transports.DatastoreRestInterceptor, "pre_begin_transaction") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = datastore.BeginTransactionRequest.pb(datastore.BeginTransactionRequest())
        transcode.return_value = {
            "method": "post",
            "uri": "my_uri",
            "body": pb_message,
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.BeginTransactionResponse.to_json(datastore.BeginTransactionResponse())
...
def test_begin_transaction_rest_bad_request(transport: str = 'rest', request_type=datastore.BeginTransactionRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_begin_transaction_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/projects/{project_id}:beginTransaction" % client.transport._host, args[1])
...
def test_commit_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.CommitResponse.to_json(datastore.CommitResponse())
...
def test_commit_rest_bad_request(transport: str = 'rest', request_type=datastore.CommitRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_commit_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/projects/{project_id}:commit" % client.transport._host, args[1])
...
def test_rollback_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("projectId", "transaction", )))
...
def test_rollback_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.RollbackResponse.to_json(datastore.RollbackResponse())
...
def test_rollback_rest_bad_request(transport: str = 'rest', request_type=datastore.RollbackRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_rollback_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/projects/{project_id}:rollback" % client.transport._host, args[1])
...
def test_allocate_ids_rest_required_fields(request_type=datastore.AllocateIdsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).allocate_ids._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_allocate_ids_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DatastoreRestInterceptor, "post_allocate_ids") as post, \
         mock.patch.object(transports.DatastoreRestInterceptor, "pre_allocate_ids") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.AllocateIdsResponse.to_json(datastore.AllocateIdsResponse())
        ...
        client.allocate_ids(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_allocate_ids_rest_bad_request(transport: str = 'rest', request_type=datastore.AllocateIdsRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_allocate_ids_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/projects/{project_id}:allocateIds" % client.transport._host, args[1])
...
def test_reserve_ids_rest_required_fields(request_type=datastore.ReserveIdsRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).reserve_ids._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
...
def test_reserve_ids_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.DatastoreRestInterceptor, "post_reserve_ids") as post, \
         mock.patch.object(transports.DatastoreRestInterceptor, "pre_reserve_ids") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        ...
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = datastore.ReserveIdsResponse.to_json(datastore.ReserveIdsResponse())
        ...
        client.reserve_ids(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()


def test_reserve_ids_rest_bad_request(transport: str = 'rest', request_type=datastore.ReserveIdsRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_reserve_ids_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1/projects/{project_id}:reserveIds" % client.transport._host, args[1])
...
def test_datastore_base_transport():
    ...
    with mock.patch('google.cloud.datastore_v1.services.datastore.transports.DatastoreTransport.__init__') as Transport:
...
def test_datastore_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.datastore_v1.services.datastore.transports.DatastoreTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_datastore_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.datastore_v1.services.datastore.transports.DatastoreTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.DatastoreTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.DatastoreGrpcTransport, transports.DatastoreGrpcAsyncIOTransport])
def test_datastore_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.DatastoreGrpcTransport, transports.DatastoreGrpcAsyncIOTransport])
def test_datastore_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.DatastoreGrpcTransport, transports.DatastoreGrpcAsyncIOTransport])
def test_datastore_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_client_with_default_client_info():
    ...
    with mock.patch.object(transports.DatastoreTransport, '_prep_wrapped_messages') as prep:
        client = DatastoreClient(
            credentials=ga_credentials.AnonymousCredentials(),
